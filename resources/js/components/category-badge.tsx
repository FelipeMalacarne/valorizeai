...
const badgeVariants = {
    lavender:
        "text-lavender bg-lavender/10 border-lavender/50 hover:bg-lavender/30",
    ...
    rosewater:
        "text-rosewater bg-rosewater/10 border-rosewater/50 hover:bg-rosewater/30",
    flamingo:
        "text-flamingo bg-flamingo/10 border-flamingo/50 hover:bg-flamingo/30",
    ...
    sapphire:
        "text-sapphire bg-sapphire/10 border-sapphire/50 hover:bg-sapphire/30",
} as { [key in Color]: string };
const CategoryBadge = ({ category, ...props }: { category: Category }) => {
    return (
        <Badge
            {...props}
            className={cn("cursor-pointer ", badgeVariants[category.color])}
        >
            {category.name}
        </Badge>
    );
