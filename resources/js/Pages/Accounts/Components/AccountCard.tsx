...
export function AccountCard({ account } : { account: Account} ){
    ...
    return (
        <Card className="w-80 hover:shadow-lg transition-shadow">
            <CardTitle>
                <div className={`relative w-full flex justify-center align-middle p-4 py-8 rounded-t-lg bg-${account.color}`}>

                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button size="icon" variant="ghost" className="h-8 w-8 absolute top-2 right-2 hover:bg-opacity-10">
                                <MoreHorizontal className={`text-background/50`} />
                                <span className="sr-only">More</span>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => console.log(true)}>Editar</DropdownMenuItem>
                            <DropdownMenuItem>Exportar</DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => console.log(true)} >Excluir</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
