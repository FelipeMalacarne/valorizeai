...
Revisa-se o estado da arte em cada um desses eixos para posicionar a contribuição deste trabalho e fundamentar a lacuna de pesquisa identificada na Introdução.
...
\begin{longtable}{@{}p{2.5cm} p{2.5cm} p{3.5cm} p{4.5cm}@{}}
    ...
    \addlinespace
    \textbf{ValorizeAI (este TCC)} & \textbf{CaaS híbrido com Cloud Run e Cloud Tasks} & \textbf{Arquitetura E2E combinando EDA (Pub/Sub), WebSockets e cache Redis com automações transacionais}. & \textbf{Validação reprodutível via Terraform + k6 + observabilidade (SLOs, logs e filas)}. \\
    \bottomrule
\end{longtable}
\end{small}
