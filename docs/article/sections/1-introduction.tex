...
Este trabalho busca responder à seguinte questão: \textit{Como uma arquitetura híbrida e elástica, composta por Cloud Run, Redis, Cloud SQL, Cloud Tasks e WebSockets dedicados, se comporta sob estresse de carga frente a SLOs definidos e como validar esse comportamento de modo reprodutível?}
...
\begin{enumerate}
    \item \textbf{Arquitetura documentada}: descreve-se o desenho completo do ValorizeAI — balanceador/CDN, Cloud Run, Redis, Cloud SQL, Cloud Tasks e Reverb — para que outros pesquisadores possam replicar o raciocínio arquitetural em aplicações transacionais.
    \item \textbf{Infraestrutura reprodutível}: disponibiliza-se o conjunto de automações (Terraform, Docker, Makefile) que reconstrói fielmente o ambiente avaliado, mantendo a linha do tempo de mudanças de infraestrutura junto ao código-fonte.
    \item \textbf{Experimentos de desempenho}: registra-se integralmente os cenários k6 (leituras e mix), com ramp-ups, parâmetros e métricas coletadas (latências, throughput, erros), assegurando transparência na validação dos SLOs.
    \item \textbf{Teste assíncrono}: documenta-se o ensaio do pipeline baseado em Cloud Tasks e workers HTTP, explicitando como o sistema responde ao acúmulo e drenagem de tarefas.
\end{enumerate}

Em conjunto, essas contribuições formam um pacote de replicação completo — código, infraestrutura e experimentos — para avaliações futuras de workloads transacionais em plataformas CaaS.

\subsection{Estrutura do Trabalho}
\label{sec:estrutura}

A organização segue esta lógica: a \textbf{Seção \ref{sec:relacionados}} revisa a literatura sobre paradigmas de execução em nuvem,
padrões arquiteturais e metodologias de validação, situando a lacuna investigada.
A \textbf{Seção \ref{sec:fundamentacao}} consolida os conceitos teóricos e técnicos que sustentam o design do sistema e os experimentos.
A \textbf{Seção \ref{sec:metodologia}} descreve a abordagem experimental, do planejamento dos SLOs ao uso de k6 e Cloud Monitoring para coleta de evidências.
As seções seguintes detalham a implementação da arquitetura ValorizeAI, discutem os resultados obtidos e encerram com conclusões e direções futuras.
