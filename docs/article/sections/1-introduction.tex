...
A organização segue esta lógica: a \textbf{Seção \ref{sec:relacionados}} revisa a literatura sobre paradigmas de execução em nuvem, padrões arquiteturais e metodologias de validação, situando a lacuna investigada. A \textbf{Seção \ref{sec:fundamentacao}} consolida os conceitos teóricos e técnicos que sustentam o design do sistema e os experimentos. A \textbf{Seção \ref{sec:metodologia}} descreve a abordagem experimental, do planejamento dos SLOs ao uso de k6 e Cloud Tasks para coleta de evidências. As seções seguintes detalham a implementação da arquitetura ValorizeAI, discutem os resultados obtidos e encerram com conclusões e direções futuras.
