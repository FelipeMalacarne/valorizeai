...
\end{itemize}
Nas rotas de leitura (consultas de transações, contas e categorias), os controladores invocam exclusivamente \textit{Queries} especializadas que exploram o cache Redis e projetam os DTOs esperados pelo front-end. Já as operações de escrita (\texttt{POST /api/transactions}, importações em lote, automações) são canalizadas por \textit{Actions} que encapsulam validações, regras de negócio e persistência, utilizando comandos explícitos que podem ser monitorados ou reprocessados. Essa separação facilita ajustes independentes: otimizações de leitura não afetam o caminho de escrita e vice-versa, e cada fluxo abriga métricas distintas no Cloud Monitoring.
