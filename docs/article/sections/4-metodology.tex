...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \node (users) at (0,0) [table] {Users\\(id, email, preferred\_currency)};
        \node (banks) at (-4.5,-2.7) [table] {Banks\\(id, code, name)};
        \node (accounts) at (0,-3.8) [table] {Accounts\\(user\_id, bank\_id, balance)};
        \node (transactions) at (0,-7.4) [table] {Transactions\\(account\_id, category\_id, amount, date)};
        \node (splits) at (0,-11.0) [table] {Transaction Splits\\(transaction\_id, category\_id, amount)};

        \node (categories) at (6.2,-3.8) [table] {Categories\\(id, user\_id, color)};
        \node (budgets) at (10.5,-1.0) [table] {Budgets\\(user\_id, category\_id, currency)};
        \node (allocations) at (10.5,-4.8) [table] {Budget Allocations\\(budget\_id, month, amount)};
        \node (embeddings) at (6.2,-7.4) [table] {Transaction Embeddings\\(transaction\_id, vector)};

        \draw (users) -- node[midway,above left]{1:N} (accounts);
        \draw (banks) -- node[midway,above left]{1:N} (accounts);
        ...
        \draw (users) to[bend left=12] node[midway,above]{1:N} (budgets);
        \draw (categories) -- node[midway,right]{1:N} (budgets);
        \draw (budgets) -- node[midway,right]{1:N} (allocations);

        \draw (categories) -- node[midway,right]{1:N} (transactions);
        \draw (categories) to[bend right=15] node[midway,right]{1:N} (splits);
    \end{tikzpicture}}
